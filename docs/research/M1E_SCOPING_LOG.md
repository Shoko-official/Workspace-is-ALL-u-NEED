...
| `EV-0119` | Looped Transformers for Length Generalization | Definition 3.1; Propositions 3.2-3.4; Sections 4.1-4.3 |
...
| `EV-0142` | Compression Barriers for Autoregressive Transformers | Definition 4; Theorems 2, 5, 6, 8, 17; Corollary 16 |
| `EV-0143` | Tensor-attention KV memory-time tradeoff | Definition 3.10; Lemma 3.11; Theorems 4.1-4.3 |
| `EV-0144` | RNN retrieval bottleneck | Sections 3, 4.2.1, 5.1-5.2; Lemma B.24 |
...
- Huang et al. is pinned to v2. The `Delta` bounds require `Delta>0` and a
  ...
  memory.
- The sparse-retrieval record is a HiLD at ICML 2025 workshop oral, not an ICML
  main-program paper. It supplies only the retrieval component of the composite.
- Looped Transformers receives `T(n)` during training and uses either that
  count or a confidence stopping rule at inference. Its positive collision is
  limited to intra-instance recurrence and adaptive depth.
...
- Haris and Onak is pinned to v1 and applies to its attention approximation and
  dimension regimes, not all persistent memory.
- The tensor-attention cache record is pinned to v2.
- The published ICALP 2024 non-adaptive-dictionary record supersedes withdrawn
  arXiv:2001.05053. The withdrawn preprint is not evidence.
...
Positive records or generic reductions cover every candidate layer:
...
- selective labels, adaptive inference, confounded or partially observed OPE,
  ...
  discovery, longitudinal mediation, and Bayes-adaptive control; and
...
The result is `8 direct / 3 composite / 10 reduced / 3 out of scope`, with zero
`DISTINCT_CANDIDATE` objects. The three out-of-scope candidates are two recycled
longitudinal write-effect objects and one relabeled generic open lower-bound
problem.

M1e therefore proposes `STOP`. The search does not establish that no original
state-compute theorem can ever exist. It establishes that none of the 24 fully
specified candidates supplies one, and that no proof or experiment is currently
authorized.
