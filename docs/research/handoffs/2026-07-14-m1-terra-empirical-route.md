...
| Primary record | Directly relevant content | Locator read |
|---|---|---|
| [Cost-Sensitive Store Routing, OpenReview:iGRGjdhl9r](https://openreview.net/forum?id=iGRGjdhl9r) | Formalizes selection among short-term, summary, long-term, and episodic stores with coverage, exact match, waste, token, and cost-sensitive objectives; evaluates oracle, heuristic, and fixed policies | §§3.2-3.4 |
...
| Cost-sensitive routing among memory stores | Cost-Sensitive Store Routing, UMA, AgeMem, GRU-Mem | Fixed/oracle/heuristic cost-sensitive store selection in Gaikwad, plus learned memory/update/exit actions with token, waste, speed, or cost objectives in the other works | One shared price vector over more action types | REJECT as empirical route; mechanism question belongs elsewhere |
